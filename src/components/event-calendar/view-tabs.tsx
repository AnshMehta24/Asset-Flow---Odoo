...
import { TCalendarView } from "@/components/event-calendar/types";
      ...
      <TabsList className="h-auto gap-2 rounded-xl p-1 w-full">
        {tabs.map(({ icon: Icon, name, value }) => {
          ...
          return (
            ...
            >
              <TabsTrigger value={value} render={<motion.div className="flex h-8 w-full items-center justify-center cursor-pointer" animate={{ filter: "blur(0px)" }} exit={{ filter: "blur(2px)" }} transition={{ duration: 0.25, ease: "easeOut" }} />}><Icon /><AnimatePresence initial={false}>
                                      {isActive && (
                                        <motion.span
